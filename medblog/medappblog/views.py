...
def getPredictions(age, anaemia, CreatiPhosph, EjectFract, highBP, SerCreat, SerSodiu, sex, smoking, followUpTime):
    
    model = pickle.load(open("careerguide.sav","rb"))
    scaled = pickle.load(open("careerguidescaler.sav","rb"))
    prediction = model.predict(scaled.transform([age, anaemia, CreatiPhosph, EjectFract, highBP, SerCreat, SerSodiu, sex, smoking, followUpTime]))
    return prediction
...
def submitmyform(request):
    ...
    #ins = HDSymptom( result,age, anaemia, CreatiPhosph, EjectFract, highBP, SerCreat, SerSodiu, sex, smoking, followUpTime )
